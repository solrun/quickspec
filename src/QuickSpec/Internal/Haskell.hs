...
canFill fillings (Hole mv) = case Map.lookup (hole_id mv) fillings of
  ...
  Just f -> trace ("filling hole " ++ (hole_id mv) ++ " with " ++ con_name f) Just $ Fun f
canFill _ (Var v) = Just (Var v)
canFill _ (Fun f) = Just (Fun f)
canFill fillings (t1 :$: t2) = case canFill fillings t2 of
  Nothing -> trace ("couldn't fill argument") Nothing
  Just t2' -> trace ("filled argument: " ++ prettyShow t2') $ case canFill fillings t1 of
    Nothing -> trace ("couldn't fill applied") Nothing
    Just t1' -> trace ("filled applied: "++ prettyShow t1') $ case tryApply (poly t1') (poly t2') of
...
allHoles (t1,t2) = allHoles' t1 (allHoles' t2 [])
  where allHoles' (Hole mv) hs = if mv `notElem` hs then hs ++ [mv] else hs
        allHoles' (t1 :$: t2) hs = allHoles' t1 (allHoles' t2 hs)
        allHoles' _ hs = hs
