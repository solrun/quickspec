...
-----------------------------------------------------------------
-- FIXME: This works when we force every occurrence of a hole to have exactly the same type,
--       if we don't it generates many instances of the same properties
...
schemaProps p cs = map (\(lt,rt) -> ([] :=>: (lt :=: rt))) $ fillHoles (schemaSides p) cs
...
allFillings holes cs = map (allFeasibleFillings cs) holes
  where allFeasibleFillings fs h = (h, filter (feasibleFill h) fs)
