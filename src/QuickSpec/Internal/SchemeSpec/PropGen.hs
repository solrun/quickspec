...
schemaProps p cs = nub $ map (\(lt,rt) -> Polymorphic.regeneralise ([] :=>: (lt :=: rt))) $ fillHoles (schemaSides p) cs
...
feasibleFill mv c = (isJust $ matchType (hole_ty mv) (con_type c)) && (typeArity (hole_ty mv) == typeArity (con_type c))
