...
---------------------------------------------
partialApp :: Int -> (Term Constant, Term Constant) -> [(Term Constant, Term Constant)]
partialApp maxArity p = [foldl partialExpand p c | c <- combos]
  where combos = crossProd [[(i,h)|i <- [0..maxArity]]| h <- allHoles p]
-- Maybe not all these options make sense?

-- TODO: Finish
partialExpand :: (Term Constant, Term Constant) -> (Int,MetaVar) -> (Term Constant, Term Constant)
partialExpand p (k,h) | k <= typeArity (hole_ty h) = p
partialExpand (lh,rh) (k,h) | otherwise = undefined
-- generate k minus (arity h) fresh variables
-- replace h with a nested application
-- what types do we want for the new (meta)vars?

appExpand :: (Term Constant, Term Constant) -> MetaVar -> (Term Constant, Term Constant)
appExpand (lh,rh) h =  (appExpand' h lh, appExpand' h rh)
  where appExpand' mv x@(Hole mv') =
          if hole_id mv' == hid
          then Fun app :$: (Hole $ MV {hole_id = hid ++ "1", hole_ty = typeVar})
               :$: (Hole $ MV {hole_id = hid ++ "2", hole_ty = typeVar}) -- :$: t2)
          else x
...
-- replace given hole with a composition of two functions
compExpand :: (Term Constant, Term Constant) -> MetaVar -> (Term Constant, Term Constant)
compExpand (lh,rh) h = (compExpand' h lh, compExpand' h rh)
  where compExpand' mv x@(Hole mv') = if hole_id mv == hole_id mv'
          then Fun comp :$:
               (Hole $ MV {hole_id = hid ++ "1", hole_ty = arrowType [typeVar] r})
               :$: (Hole $ MV {hole_id = hid ++ "2", hole_ty = arrowType as typeVar})
          else x
            where r   = if isArrowType hty
                        then typeRes hty
                        else hty
                  as  = typeArgs hty
                  hid = hole_id mv
                  hty = hole_ty mv
        compExpand' mv (t1 :$: t2) = (compExpand' mv t1 :$: compExpand' mv t2)
        compExpand' _ x = x
        comp = con "." ((.) :: (B -> C) -> (A -> B) -> A -> C)
