...
----------------------------------------------------------------------
-- * Term matching (for working with schemas)
----------------------------------------------------------------------

-- Matching for terms. Assumes that first argument may contain holes but not the second.
matchSchemaTerm :: (Eq f, SynRep f) => Term f -> Term f -> Bool
...
matchSchemaTermEnv :: (Eq f, SynRep f) => SubstEnv -> VarEnv -> [(Term f, Term f)] -> Maybe (SubstEnv,VarEnv)
